...
case
when (select version from v$instance) like '12%' and (select usage from TAB where LTRIM(product,'.') like 'Real Application Clusters') is NOT NULL and (select usage from TAB where LTRIM(product,'.') like 'Real Application Clusters One Node') is NOT NULL then
...
when (select version from v$instance) like '11%' and (select usage from TAB where LTRIM(product,'.') like 'Real Application Clusters') is NOT NULL and (select count(*) from gv$instance) = 1 then 
case 
when LTRIM(product,'.') like 'Real Application Clusters One Node' then to_char(((to_number('1','99999.99',' NLS_NUMERIC_CHARACTERS = '',.''')*&1)*&2),'990.0')||';'
when LTRIM(product,'.') like 'Real Application Clusters' then ''||';'
else to_char(((to_number(usage,'99999.99',' NLS_NUMERIC_CHARACTERS = '',.''')*&1)*&2),'990.0')||';' end
when LTRIM(product,'.') like 'Advanced Compression' and usage is NULL and 
...
else to_char(((to_number(usage,'99999.99',' NLS_NUMERIC_CHARACTERS = '',.''')*&1)*&2),'990.0')||';' end as a from TAB where product is not null order by a desc;
spool off
exit
